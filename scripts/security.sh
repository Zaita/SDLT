...
function check_composer()
{
    if [ ! -f ./composer.lock ]; then
        echo "${RED}[FAIL]"
        echo "composer.lock not found${RESET}"
        exit 1
    fi  
...
# Will print the output of `scripts/silverstripe-depcheck.py` to stdout if problems are found, and will print "[FAIL]" and exit with non-zero.
# Otherwise, prints "[OK]" and exits 0
function check_silverstripe()
{
    cmd=$( ./scripts/silverstripe-depcheck.py )
...
function check_npm()
{
    ...
    if [[ $guard -eq 0 ]]; then
       echo "${RED}[FAIL]"
       echo "No package.json files found${RESET}"
       exit 1
    fi
