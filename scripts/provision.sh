...
apt-get install -y nano libpython2.7-stdlib python-bs4 python-setuptools && npm install -g yarn
apt-get install -y python-pip && pip install feedparser

function do_composer ()
{
  su - vagrant -c "cd /vagrant && COMPOSER_HOME='/home/vagrant/' && composer install --no-interaction --no-suggest --prefer-dist"
  ...
  for dep in $( cat /vagrant/.packages ); do
    ...
    if [ "$( echo $dep | awk -F: '{print $1}' )" = "dev-y" ]; then
      su - vagrant -c "cd /vagrant && COMPOSER_HOME='/home/vagrant/' &&  composer require --dev --no-interaction --no-suggest --prefer-dist $package $version"
    else
      su - vagrant -c "cd /vagrant && COMPOSER_HOME='/home/vagrant/' &&  composer require --no-interaction --no-suggest --prefer-dist $package $version"
    fi
  done
...
# Here, at point of provisioning; we assume that whatever is passed to the function is correct (See checks in scripts/init.sh)  
function do_php ()
{
  php="php$php_ver"
  fpm="$php-fpm"
  ...
  apt-get update && apt-get install -y --allow-unauthenticated "$php" "$fpm"

  # 2). Install the desired PHP modules
  for ext in $( python scripts/phpexts.py -p $php_ver ); do
    apt-get install -y --allow-unauthenticated "php$php_ver-$ext"
  done
  ...
  # 3). Switch versions from whatever the default is
