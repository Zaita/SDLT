...
```
#> cd ~/htdocs/sites/my-project && rsync -aC ~/team-meta-package/ .
```
...
```
#> ./scripts/init.sh -n my-project -f 'My Project' -g https://gitlab.catalyst.net.nz/SilverStripe/my-project -G 1234 -w 123456 -d mysql -v 4 -p 7.1 -t '1234:abcd' -r
```
...
```
Usage: ./scripts/init.sh -n <Short Name> [ -f '<Full Name>' ] -g <VCS Url> -G <Gitlab ID> -w <WRMS ID> -d <mysql|postgres> -v <3|4> -p <5.6|7.0|7.1|7.2|7.3> -t abcd:1234 [ -r ]
```
...
* A team standard `php-cs-fixer` config (See: scripts/lintphp.sh)
* A team standard `.editorconfig` IDE/Editor config
* A team `humans.txt` file
* A team `changelog` tool
* A team package security tool (Invoked as: `./scripts/security.sh`)
* Git tab completion
...
* Node, npm and yarn
* [TODO] Catalyst devtools-flavoured gitlab CI
...
This package installs the `[phptek/sentry](https://github.com/phptek/silverstripe-sentry)` composer dependency into your project. Ensure you have created a Sentry account at sentry.catalyst.net.nz for it and updated `sentry.yml` accordingly.

## FAQ

### What's the URL of my project?

If you passed "my-project" as the `-n` flag to `scripts/init.sh`, then the URL on your dev-environment becomes:
...
### I've customised `scripts/provision.sh` now what?
