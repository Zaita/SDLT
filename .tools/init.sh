...
function do_init ()
{
    # Ensure correct permissions on host _prior_ to running `vagrant up`. This ensures that LXD can properly access "propagated" permissioned-files + dirs
...
function do_dashboard()
{
  ...
  chmod +x ./.tools/dashboard.py && ./.tools/dashboard.py \
    -s "$project" \
    -f "$project_full_name" \
    -g "$project_vcs_url" \
    -i "$project_vcs_id" \
...
if [[ "$version" -eq 4 && -z "${remove_public}" && ! -d "public" ]]; then
